...
#+RESULTS:
: definition step : ℕ → ℕ → ℕ → ℕ
: λ (a b x : ℕ),
:   @ite (x < a ∨ x > b) (@or.decidable (x < a) (x > b) (nat.decidable_lt x a) (nat.decidable_lt b x)) ℕ 0 1
...
occurrences.inhabited : inhabited occurrences
environment.inhabited : inhabited environment
expr.inhabited : inhabited expr
level.inhabited : inhabited level
format.inhabited : inhabited format
options.inhabited : inhabited options
name.inhabited : inhabited name
option.inhabited : Π (α : Type u), inhabited (option α)
string.inhabited : inhabited string
char.inhabited : inhabited char
sum.inhabited_right : Π {α : Type u} {β : Type v} [h : inhabited β], inhabited (α ⊕ β)
sum.inhabited_left : Π {α : Type u} {β : Type v} [h : inhabited α], inhabited (α ⊕ β)
unit.inhabited : inhabited unit
subtype.inhabited : Π {α : Type u} {p : α → Prop} {a : α}, p a → inhabited {x // p x}
list.inhabited : Π (α : Type u), inhabited (list α)
nat.inhabited : inhabited ℕ
prod.inhabited : Π {α : Type u} {β : Type v} [_inst_1 : inhabited α] [_inst_2 : inhabited β], inhabited (α × β)
num.inhabited : inhabited num
pos_num.inhabited : inhabited pos_num
bool.inhabited : inhabited bool
pi.inhabited : Π (α : Sort u) {β : α → Sort v} [_inst_1 : Π (x : α), inhabited (β x)], inhabited (Π (x : α), β x)
fun.inhabited : Π (α : Sort u) {β : Sort v} [h : inhabited β], inhabited (α → β)
prop.inhabited : inhabited Prop
#+end_example
...
#+RESULTS:
: Semigroup.carrier : Semigroup → Type u_1
...
#+RESULTS:
: morphism.mor : Π {S1 : Semigroup} {S2 : Semigroup}, morphism S1 S2 → ↥S1 → ↥S2
...
#+RESULTS:
: test :
:   ∀ (S1 : Semigroup) (S2 : Semigroup) (f : morphism S1 S2) (a : ↥S1), ⇑f (a * a * a) = ⇑f a * ⇑f a * ⇑f a
: test : ∀ (S1 : Semigroup) (S2 : Semigroup) (f : morphism S1 S2) (a : ↥S1), f (a * a * a) = f a * f a * f a
