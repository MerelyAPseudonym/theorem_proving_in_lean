...
: λ (x : α), h x b1 : α → α
: λ (y : β), h a1 y : β → α
: λ (x : α), p (f (f x)) (h (f a1) b2) : α → bool
: λ (x : α) (y : β), h (f x) y : α → β → α
: λ (x : α) (y : β), h (f x) y : α → β → α
: λ (x : α) (y : β), h (f x) y : α → β → α
...
: (λ (Q R S : Type) (v : R → S) (u : Q → R) (x : Q), v (u x)) α β γ g f a : γ
...
#+RESULTS:
: /private/var/folders/vt/_vxhqnb53fz1t5xcg1b59ljc0000gn/T/org-babel-lean99004gcU:1:4: warning: declaration 'curry' uses sorry
: /private/var/folders/vt/_vxhqnb53fz1t5xcg1b59ljc0000gn/T/org-babel-lean99004gcU:3:4: warning: declaration 'uncurry' uses sorry
...
#+RESULTS:
: def compose : Π (α β γ : Type), (β → γ) → (α → β) → α → γ :=
: λ (α β γ : Type) (g : β → γ) (f : α → β) (x : α), g (f x)
: def do_twice : Π (α : Type), (α → α) → α → α :=
: λ (α : Type) (h : α → α) (x : α), h (h x)
: def do_thrice : Π (α : Type), (α → α) → α → α :=
: λ (α : Type) (h : α → α) (x : α), h (h (h x))
...
#+RESULTS:
: {fst := a, snd := b} : Σ (a : α), β a
: {fst := a, snd := b}^.fst : α
: {fst := a, snd := b}^.snd : (λ (a : α), β a) ({fst := a, snd := b}^.fst)
: a
: b
...
#+RESULTS:
: cons α a (nil α) : list α
: append α (cons α a (nil α)) l1 : list α
: append α (append α (cons α a (nil α)) l1) l2 : list α
...
#+RESULTS:
: cons a nil : list α
: append (cons a nil) l1 : list α
: append (append (cons a nil) l1) l2 : list α
