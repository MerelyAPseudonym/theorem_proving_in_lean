...
#+RESULTS:
: funext :
:   ∀ {α : Type u₁} {β : α → Type u₂} {f₁ f₂ : Π (x : α), β x},
:     (∀ (x : α), f₁ x = f₂ x) → f₁ = f₂
           ...
           (λ (x : ℕ),
              eq.rec
                (eq.refl
                   (pprod.rec
                      (λ (fst : ℕ → ℕ)
                       (snd : nat.rec punit (λ (a : ℕ) (ih_1 : Type), pprod (pprod (ℕ → ℕ) ih_1) punit) x),
                         fst)
                      (nat.rec {fst := λ (a : ℕ), a, snd := punit.star}
                         (λ (a : ℕ)
                          (ih_1 :
                            pprod (ℕ → ℕ)
                              (nat.rec punit (λ (a : ℕ) (ih_1 : Type), pprod (pprod (ℕ → ℕ) ih_1) punit) a)),
                            {fst := λ (a_1 : ℕ),
                                      nat.succ
                                        (pprod.rec
                                           (λ (fst : ℕ → ℕ)
                                            (snd :
                                              nat.rec punit
                                                (λ (a : ℕ) (ih_1 : Type), pprod (pprod (ℕ → ℕ) ih_1) punit)
                                                a), fst)
                                           ih_1
                                           a_1),
                           ...
                           pprod
                             (pprod
                                (pprod.rec
                                   (λ (fst : ℕ → ℕ)
                                    (snd :
                                      nat.rec punit (λ (a : ℕ) (ih_1 : Type), pprod (pprod (ℕ → ℕ) ih_1) punit)
                                        a), fst)
                                   (nat.rec {fst := λ (a : ℕ), a, snd := punit.star}
                                      (λ (a : ℕ)
                                       (ih_1 :
                             ...
                             punit)
                        x), fst)
                   (nat.rec {fst := eq.refl 0, snd := punit.star}
                      (λ (a : ℕ)
                       (ih_1 :
                         pprod
                           (pprod.rec
...
: eq.rec 0
:   (propext
:      {mp := λ (h : true ∧ true), and.rec (λ (ha hb : true), ha) h, mpr := λ (ha : true), and.intro ha true.intro})
: 0
...
: quot r : Type
: quot.mk r a : quot r
: quot.lift f h : quot (λ (a₁ a₂ : α), r a₁ a₂) → β
...
#+RESULTS:
: quotient.exact : ∀ {α : Type u} [s : setoid α] {a b : α}, ⟦a⟧ = ⟦b⟧ → a ≈ b
...
#+RESULTS:
: strong_indefinite_description : Π {a : Sort u} (p : a → Prop), nonempty a → {x // (∃ (y : a), p y) → p x}
...
#+RESULTS:
: epsilon : Π {a : Sort u} [h : nonempty a], (a → Prop) → a
: epsilon_spec : ∀ {a : Sort u} {p : a → Prop} (hex : ∃ (y : a), p y), p (epsilon p)
