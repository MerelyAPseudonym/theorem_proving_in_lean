...
#+RESULTS:
: vector.cases_on :
:   Π {α : Type} {C : Π (a : ℕ), vector α a → Sort u_1} {a : ℕ} (n : vector α a),
:     C 0 nil → (Π {n : ℕ} (a : α) (a_1 : vector α n), C (succ n) (a :: a_1)) → C a n
...
#+RESULTS:
#+begin_example
def hide.map : Π {α β γ : Type}, (α → β → γ) → Π {n : ℕ}, vector α n → vector β n → vector γ n :=
λ {α β γ : Type} (f : α → β → γ), map._main f
def hide.map._main : Π {α β γ : Type}, (α → β → γ) → Π {n : ℕ}, vector α n → vector β n → vector γ n :=
λ {α β γ : Type} (f : α → β → γ) {n : ℕ} (a : vector α n) (a_1 : vector β n),
  nat.brec_on n
    (λ {n : ℕ} (_F : nat.below n) (a : vector α n) (a_1 : vector β n),
       (λ {n : ℕ} (a : vector α n) (a_1 : vector β n) (_F : nat.below n),
          nat.cases_on n
            (λ (a : vector α 0) (a_1 : vector β 0) (_F : nat.below 0),
               vector.cases_on a
                 (λ (a_2 : 0 = 0) (H_2 : a == nil),
                    eq.rec
                      (vector.cases_on a_1 (λ (a : 0 = 0) (H_2 : a_1 == nil), eq.rec nil (eq.symm (eq_of_heq H_2)))
                         (λ {n : ℕ} (a : β) (a_2 : vector β n) (H_1 : 0 = succ n), nat.no_confusion H_1)
                         (eq.refl 0)
                         (heq.refl a_1))
                      (eq.symm (eq_of_heq H_2)))
                 (λ {n : ℕ} (a_1 : α) (a_2 : vector α n) (H_1 : 0 = succ n), nat.no_confusion H_1)
                 (eq.refl 0)
                 (heq.refl a))
            (λ (a : ℕ) (a_1 : vector α (succ a)) (a_2 : vector β (succ a)) (_F : nat.below (succ a)),
               vector.cases_on a_1 (λ (a_2 : succ a = 0), nat.no_confusion a_2)
                 (λ {n : ℕ} (a_3 : α) (a_4 : vector α n) (H_1 : succ a = succ n),
                    nat.no_confusion H_1
                      (λ (a_eq : a = n),
                         eq.rec
                           (λ (a : vector α (succ n)) (a_1 : vector β (succ n)) (_F : nat.below (succ n))
                            (H_2 : a == a_3 :: a_4),
                              eq.rec
                                (vector.cases_on a_1 (λ (a : succ n = 0), nat.no_confusion a)
                                   (λ {n_1 : ℕ} (a : β) (a_2 : vector β n_1) (H_1 : succ n = succ n_1),
                                      nat.no_confusion H_1
                                        (λ (a_eq : n = n_1),
                                           eq.rec
                                             (λ (a_1 : vector α n_1) (a_4 : vector β (succ n_1))
                                              (_F : nat.below (succ n_1)) (H_2 : a_4 == a :: a_2),
                                                eq.rec (f a_3 a :: (_F^.fst)^.fst a_1 a_2) (eq.symm (eq_of_heq H_2)))
                                             (eq.symm a_eq)
                                             a_4
                                             a_1
                                             _F))
